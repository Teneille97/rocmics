...
library(gganimate)
library(ragg)
options(bitmapType = "cairo")    # For Windows, to prefer Cairo PNG device
library(gifski)
library(viridis)
...
uniform_theme <- theme_minimal(base_size = 14) +
  theme(
...
# Define a set of shapes to cycle through
uniform_shapes <- c(16, 17, 15, 18, 8, 4, 1, 2, 3, 5)

# Define a color palette function
get_color_scale <- function(vars) {
  # vars is vector of unique variable names (e.g. "pH", "DOC", "Ca", etc)
  n <- length(vars)
...
# Helper function to create uniform animated plot
make_uniform_anim <- function(data, date_col, x_col = "Tmt", variable_col = "Variable", y_col = "Value_norm", title_prefix) {
  unique_vars <- unique(data[[variable_col]])
  
  ggplot(data, aes_string(x = x_col, y = y_col, color = variable_col, shape = variable_col)) +
    geom_point(size = 4) +
    get_color_scale(unique_vars) +
    scale_shape_manual(values = uniform_shapes[1:length(unique_vars)]) +
    labs(
      x = "Treatment",
      y = "Normalized Value (0-1)",
      color = variable_col,
      shape = variable_col,
      title = paste0(title_prefix, " over Time: {closest_state}")
    ) +
    uniform_theme +
    transition_states(as.factor(data[[date_col]]), state_length = 1, wrap = FALSE) +
    ease_aes('linear')
}

# 1) Soil animation
soil_long <- Alldata_Soil_phEC_summary %>%
  filter(Tmt %in% valid_treatments$Tmt) %>%
  select(Date, Tmt, mean_pH, mean_EC) %>%
  pivot_longer(cols = c(mean_pH, mean_EC),
               names_to = "Variable",
               values_to = "Value") %>%
  group_by(Variable) %>%
...
animate(soil_anim, nframes = length(unique(soil_long$Date)) * 3, fps = 5, width = 600, height = 400, renderer = gifski_renderer("soil_animation.gif"))


#to be continued: save each like this for rendering the embedded file in html produced by quarto
# Create the animation object but don't display it immediately
soil_anim_gif <- animate(
  soil_anim,
  nframes = length(unique(soil_long$Date)) * 3,
  fps = 5,
  width = 600,
  height = 400,
  units = "px",
...
# Save the animation to a file explicitly
anim_save("soil_animation.gif", animation = soil_anim_gif)


# 2) Rhizon group 1 animation
rhizon_group1_vars <- c("mean_pH", "mean_DOC", "mean_DIC", "mean_Alkalinity_meq_l")

rhizon_long1 <- Alldata_Rhizon_summary %>%
  select(Sampled_on, Tmt, all_of(rhizon_group1_vars)) %>%
  pivot_longer(cols = rhizon_group1_vars,
               names_to = "Variable",
               values_to = "Value") %>%
  group_by(Variable) %>%
  mutate(Value_norm = normalize(Value)) %>%
...
rhizon_long1$Variable <- recode_factor(rhizon_long1$Variable,
                                       "mean_pH" = "pH",
                                       "mean_DOC" = "DOC",
                                       "mean_DIC" = "DIC",
                                       "mean_Alkalinity_meq_l" = "Alkalinity")
...
animate(rhizon_anim1, nframes = length(unique(rhizon_long1$Sampled_on)) * 3, fps = 5, width = 600, height = 400, renderer = gifski_renderer("rhizon_anim_group1.gif"))

# 3) Rhizon group 2 animation
rhizon_group2_vars <- c("mean_Ca_mg_l", "mean_Mg_mg_l", "mean_Na_mg_l", "mean_K_mg_l", "mean_Fe_mg_l", "mean_Al_mg_l")

rhizon_long2 <- Alldata_Rhizon_summary %>%
  select(Sampled_on, Tmt, all_of(rhizon_group2_vars)) %>%
  pivot_longer(cols = rhizon_group2_vars,
               names_to = "Variable",
               values_to = "Value") %>%
  group_by(Variable) %>%
  mutate(Value_norm = normalize(Value)) %>%
...
rhizon_long2$Variable <- recode_factor(rhizon_long2$Variable,
                                       "mean_Ca_mg_l" = "Ca",
                                       "mean_Mg_mg_l" = "Mg",
                                       "mean_Na_mg_l" = "Na",
                                       "mean_K_mg_l" = "K",
...
animate(rhizon_anim2, nframes = length(unique(rhizon_long2$Sampled_on)) * 3, fps = 5, width = 600, height = 400, renderer = gifski_renderer("rhizon_anim_group2.gif"))

# 4) PRS Macro elements animation
macro_elements <- c("Ca","Mg","K")
prs_macro_long <- PRS_filtered %>%
  select(Retrieval.Date, Tmt, all_of(macro_elements)) %>%
  pivot_longer(cols = macro_elements, names_to = "Variable", values_to = "Value") %>%
  group_by(Variable) %>%
  mutate(Value_norm = normalize(Value)) %>%
  ungroup()
...
animate(prs_macro_anim, nframes = length(unique(prs_macro_long$Retrieval.Date)) * 3, fps = 5, width = 600, height = 400, renderer = gifski_renderer("prs_macro_anim.gif"))

# 5) PRS Micro elements animation
micro_elements <- c("Fe","Mn","Cu","Zn","B","S","Pb")
prs_micro_long <- PRS_filtered %>%
  select(Retrieval.Date, Tmt, all_of(micro_elements)) %>%
  pivot_longer(cols = micro_elements, names_to = "Variable", values_to = "Value") %>%
  group_by(Variable) %>%
  mutate(Value_norm = normalize(Value)) %>%
  ungroup()
...
animate(prs_micro_anim, nframes = length(unique(prs_micro_long$Retrieval.Date)) * 3, fps = 5, width = 600, height = 400, renderer = gifski_renderer("prs_micro_anim.gif"))