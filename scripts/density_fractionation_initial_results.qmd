...
Teneille's data analysis repository is available at <https://github.com/Teneille97/rocmics>. I performed density fractionation by density fractionation using SPT and ca. 10 g soil at Max Planck Institute for Biogeochemistry.
...
The relative pool sizes were normalized to the control as (treatment-control)/control x 100%. Note that the fractions sizes are dependent (add up to 100%) so this plot shows how distribution of SOM among pools change relative to the control, not how much SOM accumulated in that pool.

```{r}
#| echo: false
#| message: false
#| label: Proportions of SOM pools - normalized
#| warning: FALSE
