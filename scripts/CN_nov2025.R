...
initial_SOC_df <- data.frame(yintercept=2.7, initial_SOC=factor(2.7))
...
ggplot(CN_data_summary,
  ...
  geom_hline(aes(yintercept=yintercept, linetype=initial_SOC), data=initial_SOC_df, show.legend=TRUE)+
  labs(
    x = "Treatment",
    y = "Mean C (%)",
    title = "Total C for all treatments"
  ) +
  theme_classic() +
  theme(
...
ggplot(CN_sel_summary,
  ...
  geom_text(
    aes(label = letters, y = mean_C + se_C + 0.1),
    size = 5,
    show.legend = FALSE
  ) +
  labs(
  ...
  geom_hline(aes(yintercept=yintercept, linetype=initial_SOC), data=initial_SOC_df, show.legend=TRUE)+
  guides(
    colour = "none"   # removes Tmt legend
  )
